...
type MaybeAsyncFunction = ()=> unknown|Promise<unknown>;

export async function checkThrowAsync(asyncFn: MaybeAsyncFunction): Promise<Error|null> {
	try {
		await asyncFn();
	} catch (error) {
		return error;
...
export async function expectThrow(asyncFn: MaybeAsyncFunction, errorCode: string|undefined = undefined): Promise<Error|null> {
	let hasThrown = false;
...
export async function expectHttpError(asyncFn: MaybeAsyncFunction, expectedHttpCode: number, expectedErrorCode: string = null): Promise<void> {
	let thrownError = null;
...
export async function expectNoHttpError(asyncFn: MaybeAsyncFunction): Promise<void> {
	let thrownError = null;
...
export async function expectNotThrow(asyncFn: MaybeAsyncFunction) {
	let thrownError = null;
	try {
		await asyncFn();
	} catch (error) {
