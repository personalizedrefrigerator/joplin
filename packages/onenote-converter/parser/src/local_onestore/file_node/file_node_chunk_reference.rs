...
impl FileNodeChunkReference {
	pub fn parse(reader: Reader, stp_format: u32, cb_format: u32) -> Result<Self> {
		let (stp, stp_value) = match stp_format {
			...
			_ => Err(ErrorKind::MalformedOneNoteData("Invalid format (reading FileNodeChunkReference.stp_value)".into()))?,
		};
		let (cb, cb_value) = match cb_format {
			...
			_ => Err(ErrorKind::MalformedOneNoteData("Invalid format (reading FileNodeChunkReference.cb_value)".into()))?,
		};

		Ok(Self {
			stp_format, cb_format, stp, cb, cb_value, stp_value,
		})
	}
}
