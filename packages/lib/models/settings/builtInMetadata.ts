...
const builtInMetadata = (Setting: typeof SettingType) => {
	...
	const output = {
		...
		'editor.showOcrText': {
			...
			section: 'note',
			appTypes: [AppType.Desktop],
			label: () => _('Markdown editor: Show OCR text for attachments'),
			description: () => _('Allows quickly accessing the OCR text for attachments from the Markdown editor using an "OCR Text" button. Screen readers can use this to describe attachments while editing notes.'),
			storage: SettingStorage.File,
			isGlobal: true,
		},
		'notes.columns': {
