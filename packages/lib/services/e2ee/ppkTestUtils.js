...
async function createTestData() {
	...
	const keySize = 2048;
	const keyPair = await (0, ppk_1.rsa)().generateKeyPair(keySize);
	const ciphertext = await (0, ppk_1.rsa)().encrypt(plaintext, keyPair);
	return {
...
async function printTestData() {
	// eslint-disable-next-line no-console
	console.info(JSON.stringify(await createTestData(), null, '\t'));
}
async function checkTestData(data, options = null) {
	options = { throwOnError: false, silent: false, ...options };
	// First verify that the data coming from the other app can be decrypted.
	const messages = [];
	let hasError = false;
	const keyPair = await (0, ppk_1.rsa)().loadKeys(data.publicKey, data.privateKey, data.keySize);
	try {
		const decrypted = await (0, ppk_1.rsa)().decrypt(data.ciphertext, keyPair);
	...
	try {
		const encrypted = await (0, ppk_1.rsa)().encrypt('something else', keyPair);
		const decrypted = await (0, ppk_1.rsa)().decrypt(encrypted, keyPair);
...
const runIntegrationTests = async (silent = false) => {
	const log = (s) => {
		if (silent) { return; }
		// eslint-disable-next-line no-console
		console.info(s);
