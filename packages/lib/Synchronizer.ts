			...
			if (syncSteps.indexOf('delta') >= 0) {
				...
				while (true) {
					...
					for (let i = 0; i < remotes.length; i++) {
							...
							if (!local) {
								...
								if (remote.isDeleted) {
									...
									if (localFoldersToDelete.has(remoteId)) {
										...
										action = null;
										reason = 'folder deletion unscheduled';
									} else if (this.api().supportsAccurateTimestamp && remote.jop_updated_time === local.updated_time) {
						...
						allActions.push(`${action}: ${content?.id ?? local?.id} (${reason})`);
						if (!action) continue;
						...
						if (action === SyncAction.CreateLocal || action === SyncAction.UpdateLocal) {
							...
							if (content.type_ === ModelType.Folder) {
								localFoldersToDelete.delete(content.id);
							}
				...
				if (!this.cancelling()) {
					...
					if (logAllActions) {
						logger.debug('all folders to have been deleted:', [...Object.values(localFoldersToDelete)]);
